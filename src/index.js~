  ...
  try {
			    ...
			    console.log("STATUS: " +res.statusCode);
   				...
   				res.on('end', function() {
    				try {
                		...
                		context.succeed(
                  			generateResponse(
                    			buildSpeechletResponse('A song in the movie ' +title + ' is ' + test, true),
                    			{}
                			 )
                		)
    ...
    for(var i = 0; i<res.length; i++)
            ...
            console.log(res);
          	...
          	//var endpoint = "https://www.tunefind.com/api/v1/artist"; // ENDPOINT GOES HERE
            //var body = "";
               context.succeed(
                  generateResponse(
                    buildSpeechletResponse('The Artist is ' + name, true),
                    {}
                  )
                )
            break;
